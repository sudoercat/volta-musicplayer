...
function renderTlist(){
  ...
  document.getElementById('ad-tl').innerHTML=curAlb.tracks.map((t,i)=>{
    const fn=t.filename||t.file||'';
    const ttl=t.title||fn.replace('.webm','');
...
function buildQ(alb){
  const u=covUrl(alb)||'';
  return alb.tracks.map((t,i)=>({
    title:t.title||(t.filename||'').replace('.webm',''),
    artist:alb.artist||alb.id.split('|')[0],
    albumName:alb.name,albumPath:alb.path,
    coverUrl:u,filename:t.filename||t.file||'',trackIdx:i,
  }));
}
...
function playT(){
  ...
  const t=queue[qi];
  aud.src=`/api/track/${t.albumPath}/${t.filename}`;
...
function renderQueue(){
  document.getElementById('np-queue').innerHTML=queue.map((t,i)=>`
    <div class="qi${i===qi?' on':''}" onclick="qi=${i};playT()">
      <div class="qi-cov">${t.coverUrl?`<img src="${t.coverUrl}" alt="">`:''}</div>
      <div><div class="qi-ttl${i===qi?' on':''}">${x(t.title)}</div><div class="qi-sub">${x(t.artist)}</div></div>
    </div>`).join('');
}
...
function renderPlTlist(){
  ...
  el.innerHTML=pl.tracks.map((t,i)=>`
    <div class="trow${qi>=0&&queue[qi]&&queue[qi].albumPath===t.albumPath&&queue[qi].trackIdx===t.trackIdx?' playing':''}" onclick="playPlFrom(${i})">
      <div class="trow-num"><span class="trow-n">${i+1}</span><div class="trow-eq"><div class="eq-b"></div><div class="eq-b"></div><div class="eq-b"></div><div class="eq-b"></div></div></div>
      <div class="trow-title">${x(t.title)}</div>
      <div></div>
...
function doSearch(){
  ...
  if(tracks.length){
    ...
    h+=tracks.slice(0,20).map((t,i)=>{
      const regKey=`srch_${i}`;
      trackReg.set(regKey,t);
      return `<div class="str" onclick="playSearch('${regKey}')">
        <div class="str-cov">${t.coverUrl?`<img src="${t.coverUrl}" alt="">`:''}</div>
        <div><div style="font-size:14px;font-weight:600">${x(t.title)}</div><div style="font-size:12px;color:var(--text2)">${x(t.artist)} · ${x(t.albumName)}</div></div>
      </div>`;
    }).join('');
  }
